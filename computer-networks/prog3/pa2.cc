...
int A_State; // The state of side A: WaitingForData = 0, WaitingForAck = 1
int A_SeqNum; // The current sequence number A expects: 0, 1
List ListArray[100]; // A buffer of packets to send to B
int ListCount;
int B_SeqNum; // The current sequence number A expects: 0, 1

int CalculateChecksum(pkt p) {

	int cs = 0;

	cs += p.seqnum + p.acknum;
...
pkt CreatePacket(char* data, int SeqNum) {
...
void A_output (msg message)
{
	...
	// Add data to buffer
	printf("A recieved message to send to B\n");

	printf("\tAdding message to buffer (%d already in buffer)\n", ListCount);
	AddDataToBuffer(message);

	// If we're in in the waiting state then send the next message
	if(A_State == 0) {

		printf("\tCreating packet\n");
		pkt p = CreatePacket(ListArray[0].Data, A_SeqNum);

		printf("\tSending packet to layer 3 A->B\n");
		tolayer3(A, p);
...
void A_input(pkt packet)
{
	...
	if(checksum != packet.checksum || packet.seqnum != A_SeqNum) {  // If there is corruption resend it

		printf("\t\tChecksum failed, resending last packet\n");
		pkt p = CreatePacket(ListArray[0].Data, A_SeqNum);	
	...
	} else if(packet.seqnum != A_SeqNum) {  // If the sequence number is wrong then this message is expired

		printf("\t\tWrong sequence number, assuming it is a repeated ACK, ignoring\n");


	} else { // Done with this message

		printf("\t\tChecksum passed, removing message from buffer and changing sequence\n");
		...
		if(ListCount > 0) { // If there is another message queued up, send it

			printf("\t\t\tMessage buffer not empty, sending next message (%d to go)\n", ListCount);
			pkt p = CreatePacket(ListArray[0].Data, A_SeqNum);	
			...
			printf("\t\t\tMessage buffer empty, waiting\n");
			A_State = 0;

			// Not waiting on a response anymore
			stoptimer(A);
...
{
	// Should have gotten an ACK by now, resend previous message to layer 3.
	printf("A timeout, resending last packet\n");
	pkt p = CreatePacket(ListArray[0].Data, A_SeqNum);	
...
void A_init (void)
{
	...
	// Start out waiting with a 0 sequence number
	A_State = 0; 
	A_SeqNum = 0;
	ListCount = 0;
...
void B_input (pkt packet)
{
	...
	printf("B recieved packet\n");

	// Check the packet for corruption
	printf("\tCalculating checksum for packet\n");
	int checksum = CalculateChecksum(packet);

	if(checksum != packet.checksum) {  // Corruption detected, get the sender to resend by sending back an ACK with bad sequence number

		printf("\t\tChecksum failed, ACKing with old sequence number\n");
		pkt p = CreatePacket(p.payload, 1 - B_SeqNum);	

		printf("\tSending packet to layer 3 B->A\n");
		tolayer3(B, p);

	} else if(packet.seqnum != A_SeqNum) {  // If the sequence number is wrong then this message is expired

		printf("\t\tWrong sequence number, assuming it is a repeated message, ignoring\n");

	} else { // Send back an ACK with good sequence number

		printf("\t\tChecksum passed and sequence number correct, ACKing with current sequence number and changing sequence number\n");
		pkt p = CreatePacket(p.payload, B_SeqNum);	
